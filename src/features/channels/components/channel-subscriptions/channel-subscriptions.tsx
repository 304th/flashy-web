...
export const ChannelSubscriptions = () => {
  ...
  const { data: following, query: followingQuery } = useChannelFollowings({
    channelId,
  });
  const { data: followers, query: followersQuery } = useChannelFollowers({
    channelId,
  });
  ...
  return (
    <div className="flex w-full items-center justify-between">
      <div
        className="flex items-center gap-2 h-9 cursor-pointer hover:opacity-80
          transition"
        onClick={() =>
          channel?.fbId &&
