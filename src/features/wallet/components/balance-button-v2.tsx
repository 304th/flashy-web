...
export const BalanceButton = () => {
  ...
  const [balanceToken, setBalanceToken] = useState<WalletToken>(() => env.IS_BROWSER ? (localStorage.getItem("balanceToken") as WalletToken || 'blaze') : 'blaze');
    ...
    <div className="relative" onMouseLeave={() => setOpen(false)}>
      ...
      <DropdownMenu modal={false} open={open}>
        <DropdownMenuTrigger asChild>
          <Button size="sm" variant="secondary" onMouseEnter={() => setOpen(true)}>
        ...
        >
          ...
          <DropdownMenuGroup className="flex flex-col gap-1">
            <DropdownMenuItem className={`transition items-center  justify-between ${balanceToken === 'blaze' ? 'bg-base-400' : ''}`} onClick={() => {
            ...
            <DropdownMenuItem className={`transition items-center justify-between ${balanceToken === 'usdc' ? 'bg-base-400' : ''}`} onClick={() => {
            ...
            <DropdownMenuItem className={`transition items-center  justify-between ${balanceToken === 'usdt' ? 'bg-base-400' : ''}`} onClick={() => {
