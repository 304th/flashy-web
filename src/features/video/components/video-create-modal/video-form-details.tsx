...
export const VideoFormDetails = ({ onClose }: { onClose: () => void; }) => {
  ...
  return <div className="flex flex-col w-[75vw] max-w-[800px] max-h-[75vh] aspect-video mx-auto">
      <div className="flex w-full grow divide-x">
        ...
        <div className="flex flex-col w-2/5 p-4 gap-4">
          <ImageUpload
            ...
            className="w-full"
            onChange={async (file) => {
              form.setValue("thumbnailUpload", file, { shouldDirty: true, shouldValidate: true });
            }}
          />
          <p className="text-lg text-white font-medium ellipsis">{title}</p>
        </div>
      </div>
      <div className="flex w-full p-4 gap-3 justify-end">
        <Button
          variant="secondary"
          onClick={onClose}
        >
