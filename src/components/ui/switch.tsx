...
>(({ className, disabled, ...rest }, forwardedRef) => {
  return (
    <SwitchPrimitives.Root
      className={cn(
        "group/switch block h-6 w-10 shrink-0 p-0.5 outline-none focus:outline-none",
        className
      )}
      ref={forwardedRef}
      disabled={disabled}
      ...
      >
        <SwitchPrimitives.Thumb
          className={cn(
            ...
            !disabled && [
              // before
              "before:absolute before:inset-y-0 before:left-1/3 before:w-5 before:-translate-x-1 before:rounded-full before:bg-white",
              "before:[mask:--mask]",
              // after
              "after:absolute after:inset-y-0 after:right-0 after:w-3 after:-translate-x-1/3 after:rounded-full after:shadow-switch-thumb",
              // pressed
