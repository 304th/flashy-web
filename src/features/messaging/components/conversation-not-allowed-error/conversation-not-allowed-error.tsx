...
export const ConversationNotAllowedError = ({ error }: { error: string }) => {
  if (error.includes("does not allow conversations")) {
    return <ChatError title="Not Friends" error={error} />;
  } else if (error.includes("blocked")) {
    return <ChatError title="Blocked" error={error} />;
  } else if (
    error.includes("is not able to receive any messages at the moment")
  ) {
    return <ChatError title="User Banned" error={error} />;
  } else if (error.includes("is not accepting direct messages at the moment")) {
    return <ChatError title="User Busy" error={error} />;
  }
};
