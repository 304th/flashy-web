  ...
  return (
    <div className="flex flex-col gap-4 justify-center w-full">
      <div className="flex w-full justify-start gap-2">
        <Button variant="secondary" onClick={() => openModal('PlaylistCreateModal')}>
          <PlusIcon />
          Create a playlist
        </Button>
      </div>
      <div className="flex gap-4 w-full justify-center">
        <div className="flex gap-4 w-full">
          <Loadable queries={[query as any]}>
            {() => !playlists || playlists?.length > 0 ? playlists?.map(playlist => <Playlist key={playlist._id} playlist={playlist} />) : <NotFound>Playlists not found</NotFound>}
          </Loadable>
        </div>
      </div>
    </div>
