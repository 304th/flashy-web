...
export const MostLikedPosts = () => {
  ...
  return <AnimatePresence initial={false}>
    {
      mostLikedPosts && mostLikedPosts.length > 0 && <motion.div className="flex flex-col p-4 h-fit w-full gap-2 rounded bg-base-250">
        <p className="text-sm text-base-800">Top 3 Posts</p>
        <div className="flex flex-col gap-3 divide-y">
          {
            mostLikedPosts.map(socialPost => <Link href={`/social/post?id=${socialPost._id}`}><MiniSocialPost key={socialPost._id} socialPost={socialPost} /></Link>)
          }
        </div>
      </motion.div>
    }
