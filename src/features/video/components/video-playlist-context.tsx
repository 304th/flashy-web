...
interface PlaylistContextType {
  ...
  playNextVideo: (currentVideoId: string, videos: VideoPost[]) => void;
...
export const PlaylistProvider = ({ children }: PlaylistProviderProps) => {
  ...
  const playNextVideo = useCallback((currentVideoId: string, videos: VideoPost[]) => {
  ...
  const playPreviousVideo = useCallback((currentVideoId: string, videos: VideoPost[]) => {
    const currentIndex = videos.findIndex(video => video._id === currentVideoId);
    if (currentIndex > 0) {
